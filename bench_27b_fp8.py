...
def run_test(kv_dtype):
    ...
    log_file = open(f"/data/src/vllm/bench_27b_{kv_dtype}_log.txt", "w")
    ...
    if not ready:
        log_file.flush()
        with open(f"/data/src/vllm/bench_27b_{kv_dtype}_log.txt") as f:
...
results = {}
for kv in ["auto", "fp8"]:
    label = {"auto": "FP16", "fp8": "FP8"}[kv]
    print(f"=== 27B-AWQ TP2 KV={label} (CUDA Graph) ===")
    p, d = run_test(kv)
    if p is not None:
...
print("\n=== RESULTS: 27B-AWQ TP2 + CUDA Graph ===")
