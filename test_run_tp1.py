"""A/B: TP=1 no-MTP — removes cross-rank collectives; if the ~67ms step
collapses, the root cause is cross-rank lockstep pacing.
v2: keep cudagraph memory estimation ON (reserves graph pool), lower util to
0.84, cap batch 2048 and max_num_seqs 128 so profiling fits on one V100."""
import os, time

LOG = "/data/src/vllm/bench_result_tp1.log"
