...
def test_pp_frame_has_fixed_shape_for_runner_limits():
    frame = PPReceiveFrame(
        generation=3,
        row_keys=keys("a", "", "b", ""),
        row_flags=torch.tensor([1, 0, 1, 0], dtype=torch.int32),
        cursors=torch.tensor([2, 0, 1, 0], dtype=torch.int32),
        sampled_tokens=torch.tensor([[10, 11, 12], [20, 21, 22], [30, 31, 32], [40, 41, 42]], dtype=torch.int32),
        draft_tokens=torch.tensor([[100, 101], [200, 201], [300, 301], [400, 401]], dtype=torch.int32),
    )
...
def test_pack_rejects_non_monotonic_frame_generation():
    frame = PPReceiveFrame(
        ...
        cursors=torch.zeros(2, dtype=torch.int32),
        sampled_tokens=torch.zeros(2, 2, dtype=torch.int32),
...
def test_pack_rejects_malformed_shape_and_mixed_devices():
    frame = PPReceiveFrame(
        ...
        row_flags=torch.zeros(2, dtype=torch.int32),
        cursors=torch.zeros(2, dtype=torch.int32),
        sampled_tokens=torch.zeros(2, 1, dtype=torch.int32),
        draft_tokens=torch.zeros(2, 1, dtype=torch.int32),
    )
    ...
    mixed = PPReceiveFrame(
        ...
        cursors=torch.zeros(2, dtype=torch.int32),
        sampled_tokens=torch.zeros(2, 2, dtype=torch.int32),
        draft_tokens=torch.zeros(2, 1, dtype=torch.int32, device="meta"),
    )
...
def test_terminating_round_aborts_and_waits_each_work_once():
    class Work:
        def __init__(self):
            self.calls = []
    ...
    first = Work()
    second = Work()
    round = PPReceiveRound(1, torch.empty(1, 1), None, None,
                           first, second, first)

    terminate_pp_round(round)

    assert first.calls == ["abort", "wait"]
    assert second.calls == ["abort", "wait"]


def test_terminating_round_handles_work_without_abort():
    class Work:
        def __init__(self):
            self.waited = False
...
def test_pack_rejects_non_int32_transport_dtype():
    frame = PPReceiveFrame(
        1, keys(""), torch.zeros(1, dtype=torch.int32),
        torch.zeros(1, dtype=torch.int32), torch.zeros(1, 1),
        torch.zeros(1, 0, dtype=torch.int32))

    with pytest.raises(ValueError, match="int32"):
        pack_pp_frame(frame, max_num_seqs=1, num_spec_tokens=0)
