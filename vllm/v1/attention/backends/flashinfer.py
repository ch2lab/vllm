...
class FlashInferBackend(AttentionBackend):
    ...
    def supports_kv_cache_dtype(cls, kv_cache_dtype: CacheDType | None) -> bool:
        if kv_cache_dtype == "nvfp4":
            # SM100: use trtllm-gen path (requires both prefill and decode support)
            if current_platform.is_device_capability_family(100):
                return (
                    supports_trtllm_attention(is_prefill=True)
                    and supports_trtllm_attention(is_prefill=False)
                )
            # SM120: use FA2 native path (no trtllm-gen cubins available)
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(
        ...
        if self.kv_cache_spec.kv_quant_mode != KVQuantMode.NONE:
            ...
            if self.is_kvcache_nvfp4:
                # SM100: requires trtllm-gen path
                # SM120: uses FA2 native path (no trtllm-gen cubins available)
                if current_platform.is_device_capability_family(100):
                    if (
                ...
                elif not current_platform.is_device_capability_family(120):
                    raise ValueError(
                        "--kv-cache-dtype nvfp4 requires SM100 or SM120."
                    )
        ...
        # Convert kv_cache_dtype for FlashInfer's plan method.
        # FlashInfer expects torch.uint8 for NVFP4, not the string "nvfp4".
        if self.kv_cache_dtype == "nvfp4":
            self.kv_data_type_for_flashinfer = torch.uint8
        else:
            self.kv_data_type_for_flashinfer = self.kv_cache_dtype
        ...
        # Page sizes >= 128 require the trtllm-gen GQA/MQA path on SM100,
        # or FA2 native path on SM120 (guaranteed by get_supported_kernel_block_sizes).
    ...
    def get_q_data_type(self, is_prefill: bool) -> torch.dtype:
        ...
        if cache_dtype == "nvfp4":
            # SM120 doesn't support FP8 query, use BF16 instead
            if current_platform.is_device_capability_family(120):
                return self.model_config.dtype
    ...
    def _get_prefill_wrapper(
        ...
        if self._prefill_wrapper is None:
            if self.use_dcp:
                ...
                # SM100 NVFP4: requires trtllm-gen backend
                # SM120 NVFP4: uses FA2 native backend (auto)
                if self.is_kvcache_nvfp4:
                    if current_platform.is_device_capability_family(120):
                        backend = "auto"
                    else:
    ...
    def build(
        ...
        if use_cascade:
            ...
            attn_metadata.cascade_wrapper.plan(
                ...
                kv_data_type=self.kv_data_type_for_flashinfer,
            )
            ...
            if prefill_use_trtllm:
                ...
                if self.use_dcp:
                    ...
                    prefill_wrapper.plan(
                        ...
                        logits_soft_cap=self.logits_soft_cap,
                        q_data_type=self.q_data_type_prefill,
                        kv_cache_dtype=self.kv_data_type_for_flashinfer,
                    ...
                    # NVFP4 trtllm kernel (SM100) only supports FP8 output;
                    # SM120 uses FA2 native path which outputs BF16.
                    if self.is_kvcache_nvfp4 and current_platform.is_device_capability_family(100):
                        o_dtype = FP8_DTYPE
                    else:
...
class FlashInferImpl(AttentionImpl):
    ...
    def __init__(
        ...
        # SM100: needs FP8 output buffer for trtllm-gen path
        # SM120: FA2 native path outputs BF16 directly, no buffer needed
        ...
        if num_prefill_tokens > 0:
            ...
            if not prefill_use_trtllm:
                ...
                if use_dcp:
                    ...
                    # SM100 NVFP4 kernel only supports FP8 output.
                    # Use a pre-allocated FP8 buffer and dequantize afterwards.
                    # SM120 FA2 native path outputs BF16 directly.
                ...
                # SM100 NVFP4 trtllm kernel only supports FP8 output.
                # Use a pre-allocated FP8 buffer and dequantize afterwards.
                # SM120 FA2 native path outputs BF16 directly, no FP8 buffer needed.
                needs_fp8_out = (self.is_kvcache_nvfp4
                                 and output.dtype != FP8_DTYPE
                                 and current_platform.is_device_capability_family(100))
                if needs_fp8_out:
                ...
                if self.is_kvcache_nvfp4:
                    # SM100 NVFP4 trtllm-gen kernel requires FP8 query.
                    # SM120 FA2 native path uses BF16 query.
                    if current_platform.is_device_capability_family(100):
                        assert attn_metadata.q_data_type_prefill == FP8_DTYPE, (
